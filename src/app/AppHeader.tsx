import {AppBar, Box, Container, Drawer, Toolbar, Typography} from "@mui/material";
...
export default async function AppHeader() {
    ...
    async function getSubjects() : Promise<FacultySubjects>{
        "use server";
        const response = await fetch('https://api.dhbw.app/courses/MA/mapped/extended');
        const data: { [key: string]: { [subj: string]: any } } = await response.json();
        ...
        const economy = subjects.filter((subject) => subject.faculty === "Wirtschaft");
    ...
    return (
        <>
            ...
            <Box
                sx={{display: {xs: 'none', sm: 'block'}, width: {sm: Defaults.drawerWidth}, flexShrink: {sm: 0}}}
            >
                <Drawer
                    variant="permanent"
                    sx={{
